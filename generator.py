...
# 文件夹路径
folder_path = "./files/碳减排数据汇总/02_政策转变碳减排"
output_folder = "./out"
...
def plot_raster_with_features(raster_path, output_path):
    with rasterio.open(raster_path) as src:
        ...
        print('vmin max', (np.nanmin(data), np.nanmax(data)), bounds)
        # 创建绘图
        ...
        # 定义比例尺长度（单位：米）
        scalebar_length = 0.4  # 设置比例尺长度为 50000 米
        ...
        # print("=== 添加文本前 ===")
        # print("坐标轴范围 - X:", ax.get_xlim())
        # print("坐标轴范围 - Y:", ax.get_ylim())
        # print("图像尺寸（英寸）:", fig.get_size_inches())
        # print("图像尺寸（像素）:", fig.get_dpi() * fig.get_size_inches())
        # 添加比例尺文字
        ...
        # 添加指北针（从图片文件加载）
        north_img = mpimg.imread("./north.png")  # 加载指北针图片
        ...
        # 保存图片
        plt.savefig(output_path, transparent=True, dpi=300, bbox_inches='tight', pad_inches=0.03)
        plt.close()
...
for tif_file in tif_files:
    ...
    try:
        ...
        # 捕获特定于 rasterio 的 IO 错误
        print(f"无法识别文件格式或文件损坏: {raster_path}. 错误: {str(e)}")
        # 记录错误后，跳过当前文件，继续下一个
    except RasterioIOError as e:
        # 捕获特定于 rasterio 的 IO 错误
        print(f"无法识别文件格式或文件损坏: {raster_path}. 错误: {str(e)}")
